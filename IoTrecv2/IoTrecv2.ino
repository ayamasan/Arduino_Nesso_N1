...
int earth[5] = {0,0,0,0,0};  // 土壌湿度センサーなら「1」
...
void loop()
{
    ...
    if(receivedFlag){
        ...
        if(state == RADIOLIB_ERR_NONE){
            ...
            if(sensdata.length() != 17){
                ...
                else if(sensdata.startsWith(header[5])){
                    sens = 0;
                    earth[0] = 1;
                }
                else if(sensdata.startsWith(header[6])){
                    sens = 1;
                    earth[1] = 1;
                }
                else if(sensdata.startsWith(header[7])){
                    sens = 2;
                    earth[2] = 1;
                }
                else if(sensdata.startsWith(header[8])){
                    sens = 3;
                    earth[3] = 1;
                }
                else if(sensdata.startsWith(header[9])){
                    sens = 4;
                    earth[4] = 1;
                }
                else{
    ...
    if(sens >= 0){  // LoRa受信
        ...
        temp[sens] = s.toInt();
        tms[sens] = tmsnow;
        draw(sens, 1, man[sens], temp[sens], earth[sens]);
...
void draw(int sens, int onoff, int man, int temp, int earth)
{
    int color1, color2;
    char str[20];
    int tt;
    ...
    if(onoff == 0){
        ...
        if(earth == 0){
            ...
            // 土壌湿度センサーの場合は、レベルをバー表示
            // 温度に土壌湿度が送られてくる
            int lvl = (temp * 52) / 1000;
            display.fillRect(40, (24+10)*sens+40, lvl, 24, color1);
            display.fillRect(40+lvl, (24+10)*sens+40, 52-lvl, 24, color2);
            if(man == 0){  // 土壌湿度センサー乾燥判定OK
                display.fillRect(100, (24+10)*sens+40, 20, 24, BLACK);
            }
            else{  // 土壌湿度センサー乾燥判定NG
                display.fillRect(100, (24+10)*sens+40, 20, 24, RED);
            }
        }
    }
